#!/usr/bin/env python3
"""Audit zero provenance using the exact v7 all-group quadrature.

This auditor is intended for the conservative v7 opacity generator.  It does
...
Planck absorption, Planck scattering, and Rosseland flux-weighted absorption
are arithmetic averages of non-negative quantities.  They are zero only when
all contributing interpolated component nodes are zero.

Total Rosseland opacity is harmonic.  A zero total-opacity interval gives an
infinite integral of W/kappa and therefore a zero harmonic mean; positive total
opacity elsewhere in the same group does not invalidate that zero.

A positive per-field grey normalization in v7 cannot change any zero mask, so
...
def load_module(path: Path):
    spec = importlib.util.spec_from_file_location("collapse_source_v7", path)
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="Conservative v7 Planck-scattering collapse Python script",
    )
    ...
    if missing:
        raise RuntimeError(
            "The supplied generator is not the conservative v7 source; "
        ...
        try:
            ...
            for local_index, group in enumerate(groups):
                ...
                if value == 0.0 and support:
                    ...
                    classification = "invalid_v7_quadrature_result"
